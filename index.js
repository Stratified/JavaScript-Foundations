...
function mortgageCalculator(name, creditScore, p, iR, y) {
  ...
  let m = principal * (numerator / denominator);
...
let rate = mortgageCalculator("Brian", 500, 200000, 0.05, 30);
...
function variableInterestRate(p, iR, y) {
  ...
  for (let j = 0; j < 10; j++) {
    ...
    iR += 0.005;
    console.log(
      `With an interest rate of ${iR.toFixed(
        3
      )} your monthly rate is: ${m.toFixed(2)}`
    );
  }
}
let rates = variableInterestRate(200000, 0.05, 30);
